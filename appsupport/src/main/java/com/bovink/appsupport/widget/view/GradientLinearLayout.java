...
/**
 * 使用GradientDrawable作为背景的LinearLayout
 *
 * @author bovink
 * @since 2018/1/14
 */
...
public class GradientLinearLayout extends LinearLayout implements View.OnTouchListener {
    ...
    private void updateBackgroundCornerRadius(TypedArray a) {
        ...
        final int radius = a.getDimensionPixelSize(
                R.styleable.GradientLinearLayout_corners_radius, 0);
        mBackgroundDrawable.setCornerRadius(radius);

        // 获取四角的半径
        final int topLeftRadius = a.getDimensionPixelSize(
                R.styleable.GradientLinearLayout_corners_topLeftRadius, radius);
        final int topRightRadius = a.getDimensionPixelSize(
                R.styleable.GradientLinearLayout_corners_topRightRadius, radius);
        final int bottomLeftRadius = a.getDimensionPixelSize(
                R.styleable.GradientLinearLayout_corners_bottomLeftRadius, radius);
        final int bottomRightRadius = a.getDimensionPixelSize(
                R.styleable.GradientLinearLayout_corners_bottomRightRadius, radius);
    ...
    private void updateBackgroundStroke(TypedArray a) {
        ...
        final int width = a.getDimensionPixelSize(
                R.styleable.GradientLinearLayout_stroke_width, 0);

        // 描边颜色
        final int color = a.getColor(
                R.styleable.GradientLinearLayout_stroke_color, Color.TRANSPARENT);

        // 虚线宽度
        final float dashWidth = a.getDimension(
                R.styleable.GradientLinearLayout_stroke_dashWidth, 0.0f);

        if (dashWidth != 0.0f) {
            // 虚线间隙
            final float dashGap = a.getDimension(
                    R.styleable.GradientLinearLayout_stroke_dashGap, 0.0f);
    ...
    private void updateBackgroundColor(TypedArray a) {
        // 填充颜色
        final int color = a.getColor(
                R.styleable.GradientLinearLayout_solid_color, Color.TRANSPARENT);
